...
module.exports = function (encoding, client, cheerio) {
  ...
  var DownloadManager = (function () {
    ...
    DownloadManager.prototype.download = function () {
      ...
      // 一時キュー内のURLを順番にダウンロード(同時処理数: concurrency)
      async.eachLimit(tmpQueue, _this.concurrency, function (url, next) {
  ...
  cheerio.prototype.download = function (srcAttrs) {
    ...
    // 同時実行数チェック
    var concurrency = parseInt(client.core.download.concurrency, 10);
    if (concurrency < 0 || concurrency > 5) {
      throw new Error('valid download concurrency range is 1 and 5');
    }
