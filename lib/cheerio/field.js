  ...
  cheerio.prototype.field = function (name, value, onNotFound) {
    ...
    if (isGet) {
      ...
      $form.find('input,textarea,select').each(function (idx) {
        ...
        // (ブラウザと同じ挙動)
        if (tools.inArray(['submit', 'image'], type)) {
          return;
        }
        ...
        if (tools.inArray(['checkbox', 'radio'], type) && !$e.attr('checked')) {
          return;
        }
        if (typeOf(val) === 'array') {
    ...
    each(values, function (v, k, o) {
      ...
      if (tools.inArray(['checkbox', 'radio'], pType)) {
        // radioの場合は指定したvalueが該当しなければ何もしない
        if (pType === 'radio') {
          realValue = String(realValue);
          var partsValues = $parts
            .map(function (idx) {
              return $(this).val();
            })
            .get();
          if (!tools.inArray(partsValues, realValue)) {
            return;
          }
        }
        ...
        $parts.untick().each(function (idx) {
          if (tools.inArray(realValue, $(this).val())) {
            $(this).tick();
          }
        });
        return;
