...
var defineNormalProperties = function (cli) {
  ...
  var property = {
    headers           : null,  // リクエストヘッダ
    timeout           : null,  // タイムアウトまでの時間(効いているかどうか不明)
    gzip              : null,  // gzip転送する/しない
    referer           : null,  // Refererを自動設定する/しない
    followMetaRefresh : null,  // <meta[http-equiv=refresh]>を検知してリダイレクトする/しない
    maxDataSize       : null,  // 受信を許可する最大のサイズ
    forceHtml         : null,  // XML自動判別を使用しない
    debug             : null   // デバッグオプション
  };
...
var CheerioHttpCli = (function () {
  ...
  CheerioHttpCli.prototype.setIconvEngine = function (icmod) {
    util.depricated('setIconvEngine() will be removed in the future. use set("iconv", value)');
