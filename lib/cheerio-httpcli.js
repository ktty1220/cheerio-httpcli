...
module.exports = {
  ...
  fetch: function (url, param, encode, callback) {
    ...
    _requestEx({
      ...
      var errInfo = { statusCode: res.statusCode, url: url };
      if (res.statusCode !== 200) {
        return callback(_fetchError('server status', errInfo), undefined, res);
      }
      if (!body) {
        return callback(_fetchError('body is undefined', errInfo), undefined, res);
      }
      ...
      return callback(err, cheerio.load(body), res);
    });
  }
};
