...
module.exports = function (encoding, client) {
  ...
  // [EXPERIMENT]
  var origMake = cheerio.prototype._make;
  cheerio.prototype._make = function () {
    var $ = origMake.apply(this, arguments);
    // load()時にセットした_documentInfoを受け渡す(_rootがない場合もあるので各要素に直埋め)
    if (this._root) {
      for (var i = 0; i < $.length; i++) {
        $[i]._documentInfo = this._root[0]._documentInfo;
      }
    }
    return $;
  };
  ...
  cheerio.prototype.click = function (callback) {
    var doc = this._root[0]._documentInfo || this[0]._documentInfo;  // [EXPERIMENT]
    var $ = cheerio;
    var $link = null;
